    ...
    public partial class frmPensionCaseFile : Form
    {
        ...
        private void frmPensionCaseFile_Load(object sender, EventArgs e)
        {
            ...
            if(_mode == Mode._Edit)
            {
                ...
                string ppo = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][17].ToString();
                string gpo = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][18].ToString();
                ...
                string spouce = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][22].ToString();
        ...
        public bool validate()
        {
            ...
            if (deTextBox22.Text == "" || deTextBox22.Text == null || String.IsNullOrEmpty(deTextBox22.Text) || String.IsNullOrWhiteSpace(deTextBox22.Text))
            {
                retval = false;

                MessageBox.Show("You cannot leave PPO number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox22.Focus();
                return retval;
            }
        ...
        private bool checkFileNotExists(string file, string dept, string subCat)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                MessageBox.Show("This PPO number is already exists for batch - " +dt1.Rows[0][0].ToString(), "B'Zer - Confirmation !", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                retval = false;
                deTextBox22.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string dept, string subCat, string state_name, string filename, string emp_name, string desg, string fileid, string bdate, string jdate, string rdate, string dDate, string psa,string section, string file_no, string ppono,string gpo, string mobile, string hrms, string spouce, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,emp_name,desg,fileid,birth_date,joining_date,death_date,retirement_date,psa_name,section,pension_file_no,ppo_fppo,gpo_dgpo,mobile,hrms_id,spouce,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','"+dept+"','"+subCat+"','" + state_name + "','"+filename+"','" + emp_name + "','"+desg+"','"+fileid+"','"+bdate+"','"+jdate+"','"+dDate+"','"+rdate+"','"+psa+"','"+section+"','" + file_no + "','" + ppono + "','"+gpo+"','"+mobile+"','"+hrms+"','"+spouce+"','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            ...
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,emp_name,desg,fileid,birth_date,joining_date,death_date,retirement_date,psa_name,section,pension_file_no,ppo_fppo,gpo_dgpo,mobile,hrms_id,spouce,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + dept + "','" + subCat + "','" + state_name + "','" + filename + "','" + emp_name + "','" + desg + "','" + fileid + "','" + bdate + "','" + jdate + "','" + dDate + "','" + rdate + "','" + psa + "','" + section + "','" + file_no + "','" + ppono + "','" + gpo + "','" + mobile + "','" + hrms + "','" + spouce + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
                //sqlCmd.Connection = sqlCon;
            }
            sqlCmd.Connection = sqlCon;
        ...
        public bool updateMetaEdit(string dept, string subCat, string state_name, string emp_name, string desg, string fileid, string bdate, string jdate, string rdate, string ddate, string psa, string section, string file_no, string ppono, string gpo, string mobile, string hrms, string spouce)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, dept, subCat, state_name, emp_name, desg, fileid, bdate,
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string dept, string subCat, string state_name, string emp_name, string desg, string fileid, string bdate,
            ...
            string ppono, string gpo, string mobile, string hrms, string spouce)
        {
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '"+ fileName + "',department  = '" + dept + "',subcat = '" + subCat + "'," +
                "state_name = '" + state_name + "',emp_name = '" + emp_name + "',desg= '" + desg + "',fileid='" + fileid + "'," +
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (validate() == true)
                    {
                        ...
                        if (checkFileNotExists(filename,dept,subCat) == true)
                        {
                            ...
                            bool insertCase = insertIntoMeta(item, dept, subCat, statename, filename, emp_name, designation, fileid, bDate, joinDate, rDate, dDate, psaOrag, section, filenumber, ppo, gpo, mobileNumber, hrms, spouce, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
            ...
            if(_mode == Mode._Edit)
            {
                if (currStage == "Entry")
                {
                    if (validate() == true)
                    {
                        ...
                        if (deTextBox22.Text.Trim() != Files.filename)
                        {
                            if (checkFileNotExistsEdit(deTextBox22.Text.Trim(),dept,subCat) == true)
                            {
                                casefile = deTextBox22.Text.Trim();
                                ...
                                bool updateMeta = updateMetaEdit(dept, subCat, statename, emp_name, designation, fileid, bDate, joinDate,
        ...
        private void deTextBox22_Leave(object sender, EventArgs e)
        {
            if (_mode == Mode._Add)
            {
                checkFileNotExists(deTextBox22.Text.Trim(), deTextBox4.Text.Trim(), deTextBox5.Text.Trim());
            }
            if (_mode == Mode._Edit)
            {
                if (currStage == "Entry")
                {
                    if (deTextBox22.Text.Trim() != Files.filename)
                    {
                        checkFileNotExistsEdit(deTextBox22.Text.Trim(), deTextBox4.Text.Trim(), deTextBox5.Text.Trim());
                    }
                }
                if (currStage == "FQC")
                {
                    if (deTextBox22.Text.Trim() != aeFQC.filename)
                    {
                        checkFileNotExistsEdit(deTextBox22.Text.Trim(), deTextBox4.Text.Trim(), deTextBox5.Text.Trim());
                    }
                }
